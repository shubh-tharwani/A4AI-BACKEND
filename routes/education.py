...
from services.visual_aid_service import generate_visual_aid as generate_visual_aid_service
...
async def create_activity(
    ...
    try:
        ...
        # Generate activity using the upgraded activities service
        activity_result = await generate_interactive_story(
            grade=request.grade,
            topic=request.topic,
            user_id=user_id
        )
        ...
        activity = {
            "title": activity_result.get("title", f"Activity: {request.topic}"),
            "instructions": activity_result.get("story_text", "Activity instructions generated by AI"),
...
async def create_visual_aid(
    ...
    try:
        ...
        # Generate visual aid using the upgraded service
        visual_aid_result = await generate_visual_aid_service(
            prompt=f"Create a visual aid for {request.concept}",
            asset_type="image",
            user_id=user_id,
        ...
        visual_aid = {
            ...
            "description": visual_aid_result.get("description", f"Visual aid for {request.concept}"),
            "image_url": visual_aid_result.get("image_url", "/static/placeholder.png"),
